...
class BookTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.review = Review.objects.create(
            book=cls.book,
            review='An excellent review! Highly recommended.',
            author=cls.user
        )
    ...
    def test_book_list_view_for_logged_in_user(self):
        # so login with email doen't work in tests
    ...
    def test_book_detail_view_with_permissions(self):
        # so login with email doen't work in tests
        # self.client.login(email='reviewuser@exmaple.com', password='testpass123')
        self.client.login(username='reviewuser', password='testpass123')
        self.user.user_permissions.add(self.special_permission)  # adding permission to testuser
        response = self.client.get(self.book.get_absolute_url())
        no_response = self.client.get('/books/1245/')
        self.assertEqual(response.status_code, 200)
